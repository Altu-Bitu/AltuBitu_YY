...
const int SIZE = 100000; //(양의)정수 최대 사이즈
...
} //에라토스테네스의 체를 응용한 소수 경로를 저장한 배열

//소인수분해해서 지수 계산하는 함수
void countExponent(int a, int cnt) {
    while (a > 1) { //소인수분해, prime[a] = 소인수, a가 1이 될때까지 진행
        exponent[prime[a]] += cnt; //연산이 곱하기라면 cnt = 1 이여서 지수 증가, 나누기라면 cnt = -1로 지수 감소
        a = a / prime[a]; //a의 소인수로 나눈 a로 다시 진행
    }
}
...
bool isRationalNumber() {
    for (int i = 2; i <= SIZE; i++) {
        if (exponent[i] < 0) //유리수라면 , 지수가 음수인 소수가 하나라도 존재한다면 유리수
            return true;
    }
    return false; // 지수가 음수인 소수가 하나도 없다면 정수
}
